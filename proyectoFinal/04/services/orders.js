...
export default class OrdersServices {
  async postOrder(userId) {
    ...
    postOrder.carrito.productos = postOrder.carrito.productos.map(prod => prod)
    ...
    if (process.env.TWILIO_SUPPORT == "true") {
      try {
        ...
        const waMessage = {
          ...
          to: "whatsapp:+5491137924505", // IF TWILLIO ACCOUNT IS PAID WE SHOULD PUT HERE user.telefono
        };
